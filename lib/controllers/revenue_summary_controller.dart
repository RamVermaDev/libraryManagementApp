...
class RevenueSummaryController {
  ...
  Future<void> getMonthlyRevenue({
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse(
          '$uri/api/$libraryId/getmonthlyrevenue?month=$month&year=$year',
        ),
        headers: {
          'Content-Type': 'application/json; charset=UTF-8',
          'Authorization': 'Bearer $token',
      ...
      if (response.statusCode == 200) {
        ...
        final monthlyRevenue = MonthlyRevenueModel.fromMap(data['monthData']);

        ref
            .read(revenueProvider.notifier)
            .setMonth(year: year, month: month, data: monthlyRevenue);
      }
      ...
      if (context.mounted) {
        showSnackBar(context, 'Unable to load monthly revenue');
      }
    }
  }
}
